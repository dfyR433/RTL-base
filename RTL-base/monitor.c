...
_Static_assert(IDB_FCS_LEN_BYTES == 0u || IDB_FCS_LEN_BYTES == 4u,
               "IDB_FCS_LEN_BYTES must be 0 or 4");
...
static void traffic_ema_bump(channel_stats_t *s, uint32_t sample) {
    uint32_t old, new_val;
    do {
        old     = __atomic_load_n(&s->traffic_ema, __ATOMIC_RELAXED);
        new_val = (old * 7u + sample) / 8u;
    } while (!__atomic_compare_exchange_n(&s->traffic_ema, &old, new_val,
...
static void ch_stat_update(uint8_t ch, int8_t rssi, uint8_t fc_type)
{
    ...
    if (s->frames_total >= 2u) {
        int64_t sum64      = (int64_t)s->rssi_sum;
        int64_t prev_sum64 = sum64 - (int64_t)rssi;
        int64_t cur_mean_q8  = (sum64      << 8) / (int64_t)s->frames_total;
        int64_t prev_mean_q8 = (prev_sum64 << 8) / (int64_t)(s->frames_total - 1u);
        int64_t x_q8         = (int64_t)rssi << 8;
        int64_t delta_cur    = x_q8 - cur_mean_q8;
        int64_t delta_prev   = x_q8 - prev_mean_q8;
        s->rssi_m2 += delta_cur * delta_prev;
    }
...
static void pool_free(uint8_t *buf) {
    ...
    uint32_t rd = __atomic_load_n(&s_free_rd, __ATOMIC_ACQUIRE);
    uint32_t wr = __atomic_load_n(&s_free_wr, __ATOMIC_RELAXED);

    if ((wr - rd) >= PACKET_POOL_SIZE) {
        configASSERT(!"monitor: pool_free ring-full — double-free?");
        return;
    }
...
/* =========================================================================
 * Lock-free SPSC ring — promisc ISR (writer)
 * ========================================================================= */
typedef struct {
    uint8_t *data;
    uint32_t orig_len;
...
static bool frame_has_a2(uint8_t fc0) {
    uint8_t type    = (fc0 >> 2) & 0x3u;
    uint8_t subtype = (fc0 >> 4) & 0xFu;
    if (type != 1u) return true;
...
static bool frame_has_a3(uint8_t fc0) {
    uint8_t type = (fc0 >> 2) & 0x3u;
    return (type == 0u) || (type == 2u);
}
static uint32_t min_len_for_a3(void) { return 24u; }
static uint32_t min_len_for_a2(void) { return 16u; }

static bool mac_match(const uint8_t *addr) {
    for (int i = 0; i < s_mac_count; i++) {
        if (memcmp(s_mac_list[i], addr, 6u) == 0) return true;
    }
    return false;
}

static bool check_mac_filter(const uint8_t *buf, uint32_t len) {
    mac_filter_mode_t mode = s_mac_mode;
    if (mode == MAC_FILTER_NONE) return true;
    if (len < 10u) return (mode == MAC_FILTER_DENYLIST);

    bool any_match = false;
    if (mac_match(buf + 4u)) any_match = true;
    if (!any_match && frame_has_a2(buf[0]) && len >= min_len_for_a2()) {
        if (mac_match(buf + 10u)) any_match = true;
    }
    if (!any_match && frame_has_a3(buf[0]) && len >= min_len_for_a3()) {
        if (mac_match(buf + 16u)) any_match = true;
    }

    return any_match ? (mode == MAC_FILTER_ALLOWLIST)
                     : (mode == MAC_FILTER_DENYLIST);
}
...
static uint16_t ch_to_freq(uint8_t ch) {
    if (ch>=1u&&ch<=13u) return (uint16_t)(2407u+5u*ch);
    if (ch==14u)         return 2484u;
    if (ch>=36u&&ch<=165u) return (uint16_t)(5000u+5u*ch);
    return 0u;
}
...
static volatile uint32_t s_mon_lifecycle = MON_IDLE;
...
static void hopper_task(void *arg) {
    ...
    while (s_monitor_running) {
        ...
        if (ci >= 0) {
            ...
            uint32_t old, new_val;
            do {
                old     = __atomic_load_n(&s_ch_stats[ci].traffic_ema, __ATOMIC_RELAXED);
                new_val = (old * 7u) / 8u;
            } while (!__atomic_compare_exchange_n(
                        &s_ch_stats[ci].traffic_ema, &old, new_val,
...
static void monitor_task(void *param) {
    ...
    while (s_monitor_running) {
        ...
        while (r != w) {
            ...
            if (pass && s_mac_mode != MAC_FILTER_NONE)
                pass = check_mac_filter(entry.data, entry.len);
            ...
            if (pass) {
                ...
                write_packet_record(&entry, entry.tsf_us);
            }
...
void monitor_start(void) {
    uint32_t exp = MON_IDLE;
    if (!__atomic_compare_exchange_n(&s_mon_lifecycle, &exp, MON_STARTING,
                                      false, __ATOMIC_ACQ_REL,
                                      __ATOMIC_RELAXED))
        return;
    ...
    __atomic_store_n(&s_mon_lifecycle, MON_RUNNING, __ATOMIC_RELEASE);
}

void monitor_stop(void) {
    uint32_t exp = MON_RUNNING;
    if (!__atomic_compare_exchange_n(&s_mon_lifecycle, &exp, MON_STOPPING,
                                      false, __ATOMIC_ACQ_REL,
                                      __ATOMIC_RELAXED))
        return;
    ...
    __atomic_store_n(&s_mon_lifecycle, MON_IDLE, __ATOMIC_RELEASE);
}
