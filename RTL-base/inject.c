 ...
 *        Scheduler: Channel-Aware Earliest Deadline First (CA-EDF) with EMA-derived slack batching.
 */
                       ...
                       inject_flags_t  flags)
{
    ...
    desc.no_ack      = (flags & INJ_FLAG_NO_ACK)       ? 1u : 0u;
...
static void stat_update(PacketInjector *inj, int64_t error_ns, uint64_t sw_ns)
{
    if (!inj->stat_init) {
        ...
        inj->stat_var_ns2   = (inj->stat_var_ns2 * 7u + abs1 * abs1) / 8u;
...
/* =========================================================================
 * EDF min-heap   (Bug #12: entry carries channel snapshot taken under lock)
 * ========================================================================= */
typedef struct {
    PacketInjector *inj;
...
static void schedulerTask(void *param)
{
    ...
    for (;;) {
        ...
        for (int i = 0; i < (int)INJECTOR_MAX; i++) {
            ...
            SchedEntry e = {
                .inj         = inj,
                .deadline_ns = inj->next_desired_ns,
                .sort_key    = edf_sort_key(inj, cur_ch),
                .channel     = inj->channel,
        ...
        PacketInjector *winner       = heap[0].inj;
        uint64_t        target       = heap[0].deadline_ns;
        uint8_t         winner_ch    = heap[0].channel;
        uint32_t        winner_gen   = heap[0].generation;

        if (winner_ch != cur_ch && winner_ch != 0u) {
            uint64_t ca_slack     = switch_slack_window_ns(cur_ch, winner_ch);
            uint64_t ca_l1_thresh = ca_slack + (ca_slack >> 1u);

            if (target > now + ca_l1_thresh) {
                uint64_t slack_lim = now + ca_slack;
                uint64_t best_dl   = UINT64_MAX;
                PacketInjector *best_inj = NULL;
                uint8_t  best_ch   = cur_ch;
        ...
        xSemaphoreTake(mgr->lock, portMAX_DELAY);
        uint64_t guard = adaptive_guard(winner);
        xSemaphoreGive(mgr->lock);

        uint64_t now2 = timer_get_time_ns_fine();
        if (target > now2 + guard) {
            uint64_t   sleep_ns = target - now2 - guard;
        ...
        timer_spin_until_ns(target);

        xSemaphoreTake(mgr->lock, portMAX_DELAY);
        PacketInjector *inj = winner;
        ...
        if ((inj->flags & INJ_FLAG_SKIP_IF_LATE) && interval_ns > 0u &&
            timer_get_time_ns_fine() > inj->next_desired_ns + interval_ns / 2u) {
            inj->stat_skips++;
            inj->swRetries_attempted = 0u;
            inj->backoff_step        = 0u;
            uint64_t elapsed         = timer_get_time_ns_fine() - inj->start_ns;
        ...
        uint8_t        snap_swr_att   = inj->swRetries_attempted;

        char name_snap[INJECTOR_NAME_MAX];
        memcpy(name_snap, inj->name, sizeof(name_snap));
        name_snap[INJECTOR_NAME_MAX - 1u] = '\0';
        ...
        uint64_t first_tx_ns  = timer_get_time_ns_fine();
        uint32_t burst_ok     = 0u;
        bool     had_tx_error = false;
        ...
        for (uint32_t b = 0u; b < burst_n; b++) {
            ...
            if (b == 0u) first_tx_ns = timer_get_time_ns_fine();
        ...
        if (burst_ok > 0u) {
            int64_t sched_error = (int64_t)first_tx_ns - (int64_t)deadline_snap;
        ...
        if (stop_asked) {
            if (inj->state == INJ_STATE_SENDING)
                inj->state = INJ_STATE_READY;
            xSemaphoreGive(mgr->lock);
            continue;
        }
...
/* =========================================================================
 * Deterministic stop — state machine
 * ========================================================================= */
static int stop_scheduler_internal(injectorManager *mgr)
{
    xSemaphoreTake(mgr->lock, portMAX_DELAY);
 ...
 * ========================================================================= */
#define SET_FIELD(field, val, bump_gen, kick_sched)                        \
    do {                                                                   \
        if (!mgr || !name) return INJ_ERR_INVALID_ARG;                     \
        xSemaphoreTake(mgr->lock, portMAX_DELAY);                          \
...
int injectorManager_setIntervalNs(injectorManager *mgr, const char *name, uint64_t iv)
{
    ...
    if (iv > 0u && inj->active) {
        uint64_t now     = timer_get_time_ns_fine();
        uint64_t elapsed = (now > inj->start_ns) ? (now - inj->start_ns) : 0ULL;
        inj->burst_idx       = elapsed / iv + 1u;
        inj->next_desired_ns = inj->start_ns + inj->burst_idx * iv;
    }
